...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        ...
        <button onclick="window.print()" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg flex items-center gap-2 text-sm transition">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <div class="metric-card">
            ...
            <div>
                ...
                <div class="metric-trend text-blue-600"><i class="fa-solid fa-arrow-trend-up text-[10px]"></i> {{ $peminjamanBulanIni > 0 ? '+' . $peminjamanBulanIni : '0' }} bulan ini</div>
            </div>
        </div>
        <div class="metric-card">
            ...
            <div>
                ...
                <div class="metric-trend text-amber-600"><i class="fa-solid fa-clock text-[10px]"></i> {{ $peminjamanMenungguBulanIni > 0 ? '+' . $peminjamanMenungguBulanIni : '0' }} bulan ini</div>
            </div>
        </div>
        <div class="metric-card">
            ...
            <div>
                ...
                <div class="metric-trend text-rose-600"><i class="fa-solid fa-circle-exclamation text-[10px]"></i> {{ $alatRusakBulanIni > 0 ? '+' . $alatRusakBulanIni : '0' }} bulan ini</div>
            </div>
        </div>
        <div class="metric-card">
            ...
            <div>
                ...
                <div class="metric-trend text-emerald-600"><i class="fa-solid fa-user-plus text-[10px]"></i> {{ $mahasiswaBaruBulanIni > 0 ? '+' . $mahasiswaBaruBulanIni : '0' }} bulan ini</div>
            </div>
        </div>
    </div>
            ...
            <div class="chart-bar-wrap">
                @foreach($alatSeringDipinjam as $alat)
                    ...
                    <div class="chart-col">
                        ...
                        <div class="chart-label" title="{{ $alat->nama_alat }}">{{ Str::limit($alat->nama_alat, 15) }}</div>
                    </div>
                @endforeach
            </div>
        @else
                ...
                <table class="w-full text-left text-sm border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100 text-slate-700">
                        ...
                        <tr class="hover:bg-slate-50/50">
                            <td class="py-3 px-5 font-semibold text-slate-800">PJM-{{ str_pad($pjm->id, 4, '0', STR_PAD_LEFT) }}</td>
                            <td class="py-3 px-5">{{ $pjm->mahasiswa->nama_lengkap ?? $pjm->mahasiswa->name ?? '-' }}<br><span class="text-xs text-slate-400">{{ $pjm->mahasiswa->nim ?? '' }}</span></td>
                            <td class="py-3 px-5">
                                @if($pjm->items->count() > 0)
                                    {{ $pjm->items->first()->tool->nama_alat ?? 'Alat Dihapus' }} ({{ $pjm->items->first()->jumlah_unit }})
                                    @if($pjm->items->count() > 1) <span class="text-xs text-slate-400">+{{ $pjm->items->count()-1 }} lain</span> @endif
                                @else
                                    -
                                @endif
                            </td>
                            ...
                            <td class="py-3 px-5">
                                ...
                                @endphp
                                <span class="px-2.5 py-1 text-[11px] font-bold rounded-full {{ $color }}">{{ $pjm->status }}</span>
                            </td>
                        </tr>
                        @empty
                        <tr><td colspan="5" class="py-6 px-5 text-center text-slate-500">Belum ada riwayat peminjaman</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100 text-slate-700">
                        ...
                        <tr class="hover:bg-slate-50/50">
                            <td class="py-3 px-5 font-semibold text-slate-800">BRG-{{ str_pad($inv->id, 3, '0', STR_PAD_LEFT) }}</td>
                            <td class="py-3 px-5">{{ $inv->nama_barang }}</td>
                            <td class="py-3 px-5 text-center font-bold text-slate-900">{{ $inv->stok }}</td>
                            <td class="py-3 px-5 text-center text-emerald-600 font-medium">{{ $inv->kondisi == 'Baik' ? $inv->stok : 0 }}</td>
                            <td class="py-3 px-5 text-center text-rose-600 font-medium">{{ $inv->kondisi != 'Baik' ? $inv->stok : 0 }}</td>
                            <td class="py-3 px-5 text-center">
                                @if($inv->stok > 10)
                                    <span class="px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-600 text-[11px] font-bold">Aman</span>
                                @elseif($inv->stok > 0)
                                    <span class="px-2 py-0.5 rounded-full bg-amber-50 text-amber-600 text-[11px] font-bold">Rendah</span>
                                @else
                                    <span class="px-2 py-0.5 rounded-full bg-rose-50 text-rose-600 text-[11px] font-bold">Habis</span>
                                @endif
                            </td>
                        </tr>
                        @empty
                        <tr><td colspan="6" class="py-6 px-5 text-center text-slate-500">Belum ada data inventaris</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100 text-slate-700">
                        ...
                        <tr class="hover:bg-slate-50/50">
                            <td class="py-3 px-5 whitespace-nowrap text-slate-500">{{ $mutasi->created_at->format('d M Y, H:i') }}</td>
                            <td class="py-3 px-5 font-semibold text-slate-800">{{ $mutasi->nama_pelaku }}</td>
                            <td class="py-3 px-5">
                                @if(str_contains(strtolower($mutasi->aksi), 'masuk'))
                                    <span class="px-2 py-0.5 rounded-md bg-emerald-50 text-emerald-600 text-[11px] font-bold inline-block mb-1">Stok Masuk</span>
                                @elseif(str_contains(strtolower($mutasi->aksi), 'keluar'))
                                    <span class="px-2 py-0.5 rounded-md bg-rose-50 text-rose-600 text-[11px] font-bold inline-block mb-1">Stok Keluar</span>
                                @else
                                    <span class="px-2 py-0.5 rounded-md bg-blue-50 text-blue-600 text-[11px] font-bold inline-block mb-1">{{ $mutasi->aksi }}</span>
                                @endif
                                <div class="text-slate-500 text-xs mt-1">Pada item ID: {{ $mutasi->id_record }}</div>
                            </td>
                        </tr>
                        @empty
                        <tr><td colspan="3" class="py-6 px-5 text-center text-slate-500">Belum ada riwayat mutasi</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
                ...
                <table class="w-full text-left text-sm border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100 text-slate-700">
                        ...
                        <tr class="hover:bg-slate-50/50">
                            ...
                            <td class="py-3 px-5 font-semibold text-slate-800">{{ $mhs->nama_lengkap ?? $mhs->name }}</td>
                            ...
                            <td class="py-3 px-5 text-center bg-slate-50/50 font-bold text-slate-900">{{ $mhs->total_pengajuan }}</td>
                        </tr>
                        @empty
                        <tr><td colspan="6" class="py-6 px-5 text-center text-slate-500">Belum ada data mahasiswa</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
